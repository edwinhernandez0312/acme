...
export class EditDriverComponent implements OnInit {
  ...
  getOwnerData(): void {
    this.http.get(`http://localhost:8000/api/conductores/${this.cedula}`).subscribe(
      (response: any) => {
  ...
  updateOwner(): void {
    ...
    this.http.put(`http://localhost:8000/api/conductores/${this.cedula}`, this.ownerData).subscribe(
      (response) => {
